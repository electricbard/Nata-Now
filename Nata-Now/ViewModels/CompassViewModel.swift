...
final class CompassViewModel: ObservableObject {
    ...
    private var previousHighlightedID: UUID?
    private var wasNearArrival = false
    ...
    private func updateHighlight() {
        ...
        // Haptic on highlight change
        if let facing, facing.id != previousHighlightedID {
            previousHighlightedID = facing.id
            highlightHaptic.impactOccurred()
        }

        highlightedLocation = facing
        let nearNow = (facing?.distance ?? .infinity) <= 100

        // Haptic on entering near-arrival zone
        if nearNow && !wasNearArrival {
            arrivalHaptic.notificationOccurred(.success)
        }
        wasNearArrival = nearNow
        isNearArrival = nearNow
    }
